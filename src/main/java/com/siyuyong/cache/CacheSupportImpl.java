...
import com.siyuyong.cache.expression.CacheOperationExpressionEvaluator;
import com.siyuyong.util.ReflectionUtils;
...
public class CacheSupportImpl implements CacheSupport {
    ...
    @Autowired
    private CustomizedConcurrentMapCacheManager cacheManager;
    ...
    public void registerInvocation(Object targetBean, Method targetMethod, Class[] invocationParamTypes,
        ...
        for (Cache cache : caches) {
            if (cache instanceof CustomizedConcurrentMapCache) {
                CustomizedConcurrentMapCache concurrentMapCache = ((CustomizedConcurrentMapCache) cache);
                // 将方法信息放到redis缓存
                // 放内存中存放
                Map<String, CachedMethodInvocation> cachedMethodInvocationMap = SingleInstance.INSTANCE.getCachedMethodInvocationMap();
                cachedMethodInvocationMap.put(getInvocationCacheKey(concurrentMapCache.getName() + key), invocation);
            }
        }
    }
    ...
    public void refreshCacheByKey(String cacheName, String cacheKey) {
        Map<String, CachedMethodInvocation> cachedMethodInvocationMap = SingleInstance.INSTANCE.getCachedMethodInvocationMap();
        CachedMethodInvocation invocation = cachedMethodInvocationMap.get(getInvocationCacheKey(cacheName + cacheKey));
        //在redis拿到方法信息，然后刷新缓存
        if (invocation != null) {
            // 执行刷新方法
    ...
    private void refreshCache(CachedMethodInvocation invocation, String cacheName) {
        try {
            ...
            CustomizedConcurrentMapCache ConcurrentCache = (CustomizedConcurrentMapCache) cache;
            // 刷新redis中缓存法信息key的有效时间
            ConcurrentCache.getCacheCreateTime().put(getInvocationCacheKey((String) invocation.getKey()), System.currentTimeMillis());
